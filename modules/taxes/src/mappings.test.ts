...
const getMappingPath = (year, form) =>
  `${root}/src/mappings/${year}/${form}.json`;
...
describe("Tax Form Mappings", () => {
  ...
  it.only(`should create any mapping files that don't exist yet`, async () => {
  ...
  it(`should create & fix one form mapping`, async () => {
    const [year, form] = [TaxYears.USA20, "f1040"];
    const mapping = await getMapping(year, form);
    const mappingPath = getMappingPath(year, form);
    let currentMapping;
    if (fs.existsSync(mappingPath)) {
      currentMapping = JSON.parse(fs.readFileSync(mappingPath)) as any;
      syncMapping(mapping, currentMapping);
    } else {
      currentMapping = mapping;
    }
    log.info(`Mapping at ${mappingPath} has ${currentMapping.length} entries`);
