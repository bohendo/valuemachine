...
const log = getLogger("info").child({ module: "f1040sd" });
const { add, eq, gt, lt, round } = math;

export const f1040sd = (taxRows: TaxRow[], oldForms: any): any => {
  ...
  const totalF8949 = f8949 => {
    const shortType =
      ...
      : f8949.P1_CC ? "C"
      : "?";
    log.info(`Short-term ${shortType} f8949 row: proceeds=${f8949.P1L2d} cost=${f8949.P1L2e} gain|loss=${f8949.P1L2h}`);
    ...
    const longType =
      ...
      : f8949.P2_CF ? "F"
      : "?";
    log.info(`Long-term ${longType} f8949 row: proceeds=${f8949.P2L2d} cost=${f8949.P2L2e} gain|loss=${f8949.P2L2h}`);
