...
describe("VM", () => {
  ...
  it("should process borrowing & repaying a loan", async () => {
    [getTestTx([
      ...
      { asset: ETH, category: Internal, from: ethAccount, quantity: "2.00", to: aaveAccount },
    ]), getTestTx([
      { asset: ETH, category: Fee, from: ethAccount, quantity: "0.01", to: Ethereum },
      { asset: DAI, category: Borrow, from: notMe, quantity: "200", to: aaveAccount },
    ]), getTestTx([
      { asset: DAI, category: Income, from: notMe, quantity: "10.00", to: aaveAccount },
    ]), getTestTx([
      { asset: ETH, category: Fee, from: ethAccount, quantity: "0.01", to: Ethereum },
    ...
    log.info(vm.json.events, "resulting events");
    expect(vm.json.events.length).to.equal(4);
    log.info(vm.json.chunks, "resulting chunks");
    // expect(vm.json.chunks.length).to.equal(8);
  });
  ...
  it("should process a trade that includes a refund", async () => {
    ...
    // log.debug(vm.json.events, "resulting events");
